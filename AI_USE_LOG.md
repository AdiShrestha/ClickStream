...
2026-07-10 | Gemini | Week 5 Extension Phase 3 | Level 3 | Audited Phase 2. Recomputed all metrics via a fresh script and confirmed 100% fidelity to JSON. Identified a conceptual evaluate-on-train data leak in the `week5extension.md` spec regarding the evaluation of the injected poisoning samples. Added PATTERN W5-1 to `antigravityrules.md` to prevent similar spec leaks.
2026-07-11 | Gemini | Week 5 Extension Phase 3 Follow-up | Level 3 | Claude identified that PATTERN W5-1 (evaluate-on-train leak) was a foundational issue originating in Week 4. Splitted `attacker_enroll` into strictly disjoint `attacker_craft_pool` (sessions 1 & 2) and `attacker_eval_pool` (sessions 3 & 4) across all runners. Re-ran the original 200-round V1 attack. Confirmed the attack effect survives the clean split (+6.06pp population delta, massive compromises for the 6 target victims). Formalized the conclusion of the Defense arc.
